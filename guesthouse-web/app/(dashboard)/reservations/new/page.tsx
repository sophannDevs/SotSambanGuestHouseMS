...
import type { GuestDto, RoomTypeDto, RoomDto, ReservationDto, CreateReservationRequest } from "@/lib/api-types";
...
// result never drifts with the browser's local offset. Only `todayIso`
// itself needs to be timezone-aware (the property's zone, not the browser's
// or a bare UTC "today"); once anchored, day-adding is timezone-agnostic.
...
export default function NewBookingPage() {
  ...
  const canCreateReservation = hasHydrated && hasPermission("reservation:edit");
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiFetch<ReservationDto>("/reservations", { method: "POST", body: JSON.stringify(body) });
    },
  ...
  if (hasHydrated && !canCreateReservation) {
    return (
      <div className="p-4 md:p-0 md:mt-6">
        <EmptyState
          ...
          action={{ label: t("noPermission.backAction"), onClick: () => router.push("/reservations") }}
        />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-lg md:max-w-3xl">
      ...
      <div className="p-4 space-y-5 pb-48 md:pb-32">
        {step === 1 && (
      ...
      <FormActions sticky className="mx-auto max-w-lg md:max-w-3xl">
        {step > 1 && (
